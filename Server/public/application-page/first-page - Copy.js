function renderRegistrationForm() {
  return `<div class='main__container'>
        <div class='container'>
            <h3 class="welcome-message">welcome ${getInformationFromCookies().firstname}</h3>
            ...
            <form class='main'>
                ...
                <div class="main__input-group">
                  ...
                  <select class="main__select" name="highest-degree">
  ...
  for (let i = 0; i < cookiesInfo.length; i++) {
    ...
    if (targetedInput) {
      if (targetedInput.type === "radio") {
        const inputRadios = Array.from(document.querySelectorAll(`input[name=${targetedInput.name}]`));
  ...
  fetch("http://localhost:6000/chamsbootcamp/candidate-application", option)
    ...
    .then((result) => {
      if (result.affectedRows === 1) {
        window.location = "http://localhost:6000/candidate-dashboard";
      }
    });
}
...
// to store the data in the post body
// if(result.status == 200) store the data in cookie
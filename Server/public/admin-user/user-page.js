...
function deleteEmail() {
  ...
  adminTemplate.addEventListener("click", (event) => {
    ...
    document.getElementById("popup__yes-btn").addEventListener("click", (event) => {
      fetch(`http://localhost:6000/chamsbootcamp/register/${deleteBtn.parentElement.dataset.index}`, { method: "DELETE" }).then((response) => {
          ...
          fetch("http://localhost:6000/users")
            ...
            .then((result) => {
              ...
              if (usersFromDB.length > 0) {
                ...
                document.querySelector(".main__admin-template").innerHTML = "";
              }
            });
        }
      });
...
// function renderPreviousNextButtons try to change its name or add comment to explain its job
